...
- GGML is accepted for now to ensure verified downloads; plan to migrate to GGUF when a stable repo + checksums are locked.
- Bundle a small local ASR runtime and auto-point the command path when models are installed or preset changes.
...
- Else if RAM ≥12 GB → recommend Balanced (base).
...
| 2026-02-01 | v0.9 | Runtime ASR local bundlé + auto-config command | Dev |
...
- Bundled local ASR runtime path auto-configuration (prefers bundled runtime over PATH).
...
Review based on story documentation and file list (main.js, preload.js, dashboard.html, dashboard.js). Manual verification is noted; automated QA execution not recorded. Local VAD/ASR path with model install/upgrade and offline dictation.
...
- [ ] Add tests for model install/upgrade/uninstall flows and fallback.
- [ ] Add offline/local path validation to ensure no cloud calls.
- [ ] Add performance checks for local inference latency.

### Security Review
Local models should be validated; not re-verified in this pass.
...
### Gate Status
Gate: CONCERNS -> docs/qa/gates/13.1-local-vad-asr-path.yml
