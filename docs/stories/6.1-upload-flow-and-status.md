...
- [ ] Fix queue → processing transition (stuck “queued” regression) (AC: 3)
- [ ] Make cancel reliable and always available; cancel deletes transient note and leaves no history entry (AC: 4)
- [ ] Persist transcript + metadata in Notes (and never sync local file paths) (AC: 5,6)
- [ ] Persist failed upload records with file reference + error for later review (AC: 6)
...
- [ ] Add structured manual test script for upload flow + cancellation edge cases (AC: 1–8)
