...
- [x] Fix diagnostics summary layout for long lines (wrap + no overflow) and keep “copy diagnostics” usable (AC: 8)
...
| 2026-02-01 | v0.3 | Wrap diagnostics output to avoid horizontal overflow | Dev |
...
- Adjusted diagnostics output wrapping to prevent horizontal overflow on long lines.
- Tests: node test-core.js (pass); skipped node test-keyboard.js (requires focused text field).
- Manual verification: user confirmed Diagnostics layout verified after long-line wrap change.
