...
- [ ] Define Whisper preset matrix (Lite/Quality/Ultra) for FR+EN with capability thresholds (AC: 5,6)
- [ ] Implement a local Model Manager (manifest, download, checksum, versioning, storage, cleanup) (AC: 6,7)
- [ ] Add onboarding step for local model opt-in + download UX (AC: 5,6)
- [ ] Add settings UI to manage models (install/upgrade/remove) and show active preset (AC: 6,7)
- [ ] Wire preset selection into local ASR config and reuse for long-form local transcription when available (AC: 7)
