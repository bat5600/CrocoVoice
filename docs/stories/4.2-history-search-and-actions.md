...
4.1. History row hover actions are limited to **Copy** and **Delete** (no Export action on rows; exports live in the history detail/export flow).
...
- [x] Remove “Export” from history row hover actions (keep export in detail/export flow) (AC: 4.1)
...
| 2026-02-01 | v0.2 | Video review: remove Export from history row actions | PM |
| 2026-02-01 | v0.3 | Manual verification: hover actions + export detail availability | Dev |
...
- Retrait du bouton Export des actions hover de l’historique.
- Vérification manuelle (Windows): actions hover = Copier/Supprimer, export toujours dispo via détail/export.
...
Review based on story documentation and file list (main.js, preload.js, dashboard.js). Manual verification is noted; automated QA execution not recorded. History list UX, search/debounce, copy/delete/undo, offline behavior.
...
- [ ] Add tests for search/debounce and title derivation.
- [ ] Add tests for delete + undo persistence and offline behavior.
- [ ] Add regression test for copy toast + clipboard fallback.

### Security Review
No security-sensitive changes.

### Performance Considerations
List/search should remain responsive at ~200 entries; verify.

### Files Modified During Review
None (QA artifacts only).

### Gate Status
Gate: CONCERNS -> docs/qa/gates/4.2-history-premium-list-search-copy-delete-with-undo.yml
