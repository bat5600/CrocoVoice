...
4. Retention rules are enforced (time-based or size-based) with user controls.
5. Redaction rules apply before any sync or AI requests.

## Tasks / Subtasks
- [ ] Add context signal registry and settings UI (AC: 1,2,3)
- [ ] Implement retention policy (TTL or size caps) for stored context (AC: 4)
- [ ] Implement redaction pipeline before AI/sync (AC: 5)
