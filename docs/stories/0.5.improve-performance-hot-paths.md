...
Review based on story documentation and file list (main.js, renderer.js, store.js). Manual verification is noted; automated QA execution not recorded. IPC payload change, DB indexes, settings write optimization, and dictionary cache.
...
- [ ] Add performance benchmarks for long recordings and list load time.
- [ ] Add tests for dictionary cache invalidation and settings single-key updates.
- [ ] Validate index creation on existing DBs without side effects.

### Security Review
No security-sensitive changes.

### Performance Considerations
Perf improvements expected; verify with measurements.

### Files Modified During Review
None (QA artifacts only).

### Gate Status
Gate: CONCERNS -> docs/qa/gates/0.5-improve-performance-hot-paths.yml
