...
## Tasks / Subtasks
- [ ] Confirm dictionary UI supports add + delete with clear validation (AC: 1,2,7)
- [ ] Update dictionary schema to include usage metadata (AC: 4)
- [ ] Track usage on apply (increment count, set last_used/source/auto_learned) (AC: 4)
- [ ] Define and implement matching rules and conflict resolution (AC: 5)
- [ ] Ensure dictionary is applied in the main pipeline for all dictation entry points (AC: 3)
- [ ] Ensure offline-first behavior and non-blocking sync (AC: 6)
