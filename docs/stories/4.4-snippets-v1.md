...
3. Snippet matching rules are simple and documented (v1): exact match or prefix match on the cue after normalization (trim, collapse whitespace, lowercase, strip trailing punctuation).
4. When a dictation contains a matching cue (any position), the cue is removed from the delivered text and replaced by the template in-place.
5. The remaining dictation text (after removing the cue) is preserved after the inserted template.
...
8. Cue normalization is consistent across inputs and ignores intra-word punctuation (ex: “e-mail” matches “email”).
9. Snippets UI explains matching behavior clearly and shows an in-sentence example (no placeholder).

## Tasks / Subtasks
- [x] Add SQLite storage for snippets (create/list/delete) (AC: 1,2,7)
- [x] Add dashboard UI for managing snippets (AC: 1,2)
- [x] Implement cue matching + replacement in main dictation pipeline (AC: 3,4,5,6,8)
...
- [x] Improve snippet UX: explain in-sentence replacement + example (AC: 9)
...
- Apply snippet expansion in main process after transcription post-process and before delivery; recommended order for best UX:
  ...
  2) Snippet expansion (cue → template)
  3) Dictionary corrections
  4) Persist + deliver
- Keep matching rules conservative to avoid surprising replacements; cues can appear mid-dictation.

### Testing
- Manual: add snippet, speak cue-only, cue+extra text, cue mid-sentence, punctuation variant (ex: "e-mail"), non-matching, overlapping cues, multiple snippets.
