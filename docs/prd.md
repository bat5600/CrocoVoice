...
CrocoVoice is an **AI voice OS for writing**: a hotkey-first desktop app that captures speech, transcribes it, cleans/structures the text, and reliably **delivers** the result (type/paste/clipboard/integrations) with a premium UX.
...
- Integrations: adapter layer + graceful degradation; then specific integrations (Slack/Notion/etc.).
...
| M2 — Integrations v1 | Epic 5 + Epic 8 | NEXT | Adapter layer + 1–2 hero integrations + basic intent routing for a few actions |
...
**Out of scope**
- New major product features (meetings, integrations, search).
...
### Epic 5 — Integrations Platform & Routing (`NEXT`)
**Goal**
- Deliver text to external apps/services reliably through a clean adapter layer, with graceful degradation.

**In scope**
- Integration adapter interface; connection management; retries/backoff; user-visible status.
- “Always deliver something” policy (fallback to clipboard/paste).
- One or two hero integrations (to be decided: Slack, Notion, Gmail).

**Out of scope**
- A marketplace/SDK; dozens of integrations; deep 2-way sync.

**Shard plan**
- `docs/stories/epic-5-integration-adapter-and-reliability.md`
- `docs/stories/epic-5-slack-integration-v1.md`
- `docs/stories/epic-5-notion-integration-v1.md`

**Exit criteria**
- Integration failures are recoverable and never lose the dictation output.
- Clear UX for connected/disconnected states and errors.

**Dependencies**
- OAuth/token storage strategy; network resilience.

**Risks**
- API churn; rate limits; users perceiving “slow” behavior without loading states.
...
**In scope**
- Intent detection for a small, explicit action set (e.g., “send to Slack”, “create a note”, “copy”, “open settings”).
- Confirmation gates for risky actions.
- Routing to existing integration adapters.
...
**Dependencies**
- Epic 5 adapter layer; Epic 1 telemetry and reliability.
